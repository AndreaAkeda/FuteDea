...
export const TeamPanel = ({ team, teamName, stats, onAddEvent, isDarkMode }: TeamPanelProps) => {
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-2 sm:p-4 border shadow-lg transition-all duration-300`}>
      <div className="text-center mb-2 sm:mb-4">
        <h2 className={`text-base sm:text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-1`}>
          {teamName}
        </h2>
        <div className={`text-lg sm:text-2xl font-bold ${team === 'home' ? 'text-blue-600' : 'text-red-600'}`}>
      ...
      <div className="grid grid-cols-1 gap-1 mb-3 sm:mb-4">
        {eventButtons.map((button) => (
          <Button
            ...
            className={`${button.color} text-white text-xs py-1 px-1 h-6 sm:h-8 transition-all duration-300 hover:scale-105 active:scale-95`}
            size="sm"
          >
            {button.label}
          </Button>
      ...
      {/* Quick Stats */}
      <div className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-2 transition-colors duration-300`}>
